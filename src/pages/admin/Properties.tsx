...
import { useAllImoveis, useUpdateImovel, useDeleteImovel } from '@/hooks/use-imoveis';
...
export default function PropertiesPage() {
  ...
  const { data: imoveis, isLoading } = useAllImoveis(search, filterPublicado, filterDestaque, filterTipo, filterFinalidade);
  ...
  const togglePublicado = async (id: string, current: boolean) => {
    await updateImovel.mutateAsync({ id, publicado: !current } as any);
    toast.success(!current ? 'Imóvel publicado' : 'Imóvel despublicado');
  };

  const toggleDestaque = async (id: string, current: boolean) => {
    await updateImovel.mutateAsync({ id, destaque: !current } as any);
            ...
            <div key={imovel.id} className="flex items-center gap-4 p-4 rounded-lg border border-border bg-card hover:shadow-sm transition-shadow">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <Badge variant="outline" className="text-xs shrink-0">{TIPO_LABELS[imovel.tipo]}</Badge>
                  <Badge variant="outline" className="text-xs shrink-0">{FINALIDADE_LABELS[imovel.finalidade]}</Badge>
                </div>
              ...
              <div className="flex items-center gap-4 shrink-0">
                <div className="flex items-center gap-1.5">
                  ...
                  <Switch checked={imovel.publicado} onCheckedChange={() => togglePublicado(imovel.id, imovel.publicado)} />
                </div>
                <div className="flex items-center gap-1.5">
                  <span className="text-xs text-muted-foreground">Dest.</span>
                  <Switch checked={imovel.destaque} onCheckedChange={() => toggleDestaque(imovel.id, imovel.destaque)} />
                </div>
              </div>

              <div className="flex items-center gap-1 shrink-0">
                <Link to={`/admin/imoveis/${imovel.id}`}>
                  ...
                  <AlertDialogContent>
                    ...
                    <AlertDialogFooter>
                      ...
                      <AlertDialogAction
                        ...
                        onClick={async () => {
                          try {
                            await deleteImovel.mutateAsync(imovel.id);
